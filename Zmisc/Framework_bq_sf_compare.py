...
def execute_salesforce_query(sql_query, output_file):
    sf = Salesforce(
        username='ratna-kumar.dangeti@lloydsbanking.com.devpoc',
        password='Sony@4076',
        consumer_key='3MVG9od6vNol.eBijuSl01nDrsc_Ngz.DmtOGEn0Ja4DdL75sh_rzjhKqKo0C1DVygt2QyQ1xeGkrwtkfRE1L',
        consumer_secret= 'FB008DFDC0E87E35543FBD64DB3F5EA2B7AD7789E423138986F0B4F4203C8B03',
        domain='test'
    )
...
def generate_summary_report(sf_csv, bq_csv, comparison_result):
    ...
    return summary
def append_to_master_summary(test_case_name, summary):
    master_summary_file = "results/master_summary.csv"
    summary['TC#'] = get_next_serial_number(master_summary_file)
    summary['Test Case Name'] = test_case_name
...
def generate_comparison_report(test_case_name, source_folder, sf_csv, bq_csv, timestamped_folder):
    ...
    append_to_master_summary(test_case_name, summary)
